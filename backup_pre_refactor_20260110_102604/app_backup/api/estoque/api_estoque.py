...
async def read_produtos_from_mongo_db_beanie(cod_marca: int, data_movto_ini: str, data_movto_fim: str):
    if cod_marca != 0:
        produtos = await ProdutoEstoqueMongoBeanie.find({'cod_marca': cod_marca,'estoque_history.data_movto': {'$gt': data_movto_ini}, 'estoque_history.data_movto': {'$lt': data_movto_fim}}).to_list()
        # TODO to filter some elements of the array out one can use the aggregation sample, or try to filter on frontend
        # https://stackoverflow.com/questions/4701427/mongodb-filtering-the-content-of-an-internal-array-in-a-resultset
        # https://stackoverflow.com/questions/39217005/filtering-an-array-inside-an-array-in-mongo-db


    else:
        produtos = await ProdutoEstoqueMongoBeanie.find({'cod_marca': {'$ne': 0}}).to_list()
...
@router.get("/api/estoque/update_produtos_from_postgres_to_beanie/{cod_marca}/{dat_movto_ini}",
...
async def update_produtos_from_postgres_to_beanie(cod_marca: str, dat_movto_ini: str):
    ...
    if dat_movto_ini == ' ':
        ...
        dat_movto_ini = dat_ult_atualizacao
    ...
    for i in range(len(marcas[0:10])):
        ...
        for produto in produtos_estoque_list:
            ...
            # print('No product should enter this else after deleting produto_estoque_mongo')
            # print("db_produto"+ str(db_produto)+ '=' "produto"+ str(produto.id_movto))
            dat_ult_atualizacao = db_produto.dat_last_movto
            await dat_ult_atualizacao.save()
            await db_produto.save()

    return jsonable_encoder(produtos_da_marca)  # TODO trocar jsonable_encoder e também tudo para Beanie
